...
envfile = conf.File(path=".env", mandatory=True)
secretfile = conf.File(path=".secret.env")


FILES = [envfile, secretfile]
SECRET_DESTINATION = Path("tests/to_delete")
...
class COMPOSE_VARIABLES(conf.Section):
    ...
    HOST_NAMES = fields.StringListField(
        ...
        secret=False,
        services=[
            Service("django", user=0, group=0, mode=0o400),
            Service("nginx", user=0, group=0, mode=0o400)
        ]
    )
...
#         services=["nginx"]
#     )
#     CERTIFICATE_CRT = fields.SSLCertificateField(
#         secret=True,
#         file=secretfile
#     )
#
